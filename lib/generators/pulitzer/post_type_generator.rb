  ...
  class PostTypeGenerator < Rails::Generators::Base
    ...
    def build_template
      ...
      post_type_versions.each do |ptv|
        ...
        all_element_types.each do |el_type|
...
<!-- BEGIN #{layout_short_path} -->
        ...
        content_element_names.each do |content_element_name|
          inject_into_file layout_path(layout_name), before: "<%# END Auto-generated by Pulitzer %>", force: false do
            <<-VIEW
    <%= render_cms_element partial.content_element('#{content_element_name}') %>
...
<!-- END #{layout_short_path} -->
    ...
    def layout_path(layout_name)
      File.join(Rails.root,'app','views', Pulitzer.partial_folder, action_name, layout_name, 'v_1', "#{}.html.erb")
    end
